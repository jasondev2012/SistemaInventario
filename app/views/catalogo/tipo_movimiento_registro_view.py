...
class TipoMovimientoRegistroView(ttk.Frame):
    def __init__(self, master, intTipoMovimientoID, on_register_success):
        ...
        unidadMedida = None
        if intTipoMovimientoID > 0:
            unidadMedida = TipoMovimientoService.obtener(intTipoMovimientoID)
        ...
        ttk.Button(
            ...
            bootstyle="success",
            command=lambda: self.guardar_cambios(intTipoMovimientoID)
        ).grid(row=3, column=0, columnspan=2, padx=5, pady=20)

        self.rowconfigure(0, weight=1)
        self.columnconfigure(0, weight=1)
        
    def guardar_cambios(self, intTipoMovimientoID):
        response = TipoMovimientoService.registrar(
